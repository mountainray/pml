...
In a similar fashion, the R function `nearzeroVariance` was used to identify variables that would add little additional information to our models.  We again examined the output and concluded these would be left out.  This process identified `r nrow(filter(varmap, is.na(nzv_no)==F))` variables with zero variance, these were removed.
...
observations<-nrow(no_final)
variables<-ncol(no_final)
knitr::kable(data.frame(head(no_final[1:5,1:6])), caption = "Sample Print of final dataset")
knitr::kable(data.frame(observations, variables), caption = "Dimensions")
knitr::kable(data.frame(count(no_final, classe)), caption = "Distribution of classe variable")
```
...
Since processing time is of some consideration here, we *explored* a smaller (`r testrun_portion*100`/`r (1-testrun_portion)*100`random sample of the training data for the GBM model grid, and gave the faster treebag and ldabag models a more realistic 70/30 training/validation split.  
...
fitControl<-trainControl(method = "repeatedcv", number = 2, repeats = 2, allowParallel = T)
